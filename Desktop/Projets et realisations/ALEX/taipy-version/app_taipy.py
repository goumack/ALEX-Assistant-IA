...
class ALEXProClient:
    ...
    def is_greeting_or_general(self, message: str) -> bool:
        ...
        greetings = [
            ...
            'coucou', 'yo', 'wesh', 'ça va', 'comment ça va',
            'comment allez-vous', 'bonne journée', 'bonne soirée'
        ]
        
        # Questions générales
        general_questions = [
            'qui es-tu', 'que fais-tu', 'comment ça marche',
            'aide', 'help', 'comment utiliser', 'fonctionnement',
        ...
        if any(q in message_lower for q in general_questions):
            return True
            
        # Messages très courts (moins de 10 caractères)
        if len(message_lower) < 10 and any(word in message_lower for word in ['salut', 'hello', 'hi', 'hey']):
            return True
